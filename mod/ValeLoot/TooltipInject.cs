...
internal static class TooltipInject
{
    ...
    public static bool Install(Action<string> log)
    {
        ...
        try
        {
            ...
            // OnPointerExit is optional for compatibility with a future client. If present it prevents an
            // async provider completion from refreshing a pooled panel after the cursor has already left.
            ...
            log($"tooltip inject ready (OnPointerEnter + TMP_Text.set_text; Data 0x{_dataFieldOffset:x}, "
              + $"floors {MinTooltipChars}/{MinExternalTooltipChars} chars, async refresh {(onExit is null ? "without exit guard" : "ready")})");
        }
        catch (Exception e)
        {
...
public static class ValeLootTooltipApi
{
    ...
    /// <summary>
    /// Ask ValeLoot to rebuild the tooltip that is currently under the pointer. Intended for a registered
    /// provider whose result arrived asynchronously after the original OnPointerEnter write.
    /// </summary>
