...
### `ValeLoot-0.5.1-with-BepInEx.zip` — take this one
...
### `ValeLoot-0.5.1.zip` — the plugin on its own
...
[Info   :  ValeLoot] item catalog bound, waiting for the client to load configs (App.ServerRuntime 0x18, Equips 0x20, Artifacts 0x48, Cards 0x28, Gems 0x30, Consumables 0x38, Junks 0x10, ...)
...
```
[Info   :  ValeLoot] catalog ready: 647 equips, 45 artifacts, 327 cards, 129 gems, 31 consumables, 280 junk
...
`TopRolls` counts lines whose displayed value reaches the legal maximum. `HighRolls` counts hidden
raw rolls at or above `Threshold`. Use the one that matches the question you actually mean:
...
**Upgrading from 0.3.0 or earlier:** rename an old `TopRolls` condition to `HighRolls` when it was
intended to use `Threshold`. Keep `TopRolls` when the intended rule is “the tooltip shows the
maximum.” `AvgRoll` remains accepted as an alias for `AvgRollPct`.
